...
Personal dotfiles managed with **GNU Stow**. Each top-level directory is a stow package whose internal tree mirrors `$HOME` (e.g. `ghostty/.config/ghostty/config` → `~/.config/ghostty/config`, `git/.gitconfig` → `~/.gitconfig`). There is no build, lint, or CI; `dot_scripts/test_zsh.sh` validates the Zsh package.

## Commands

- `./dot_scripts/stow_all.sh` — stow every package into `~/` (auto-globs top-level dirs and skips `dot_scripts/`).
...
- `./dot_scripts/install_zsh.sh` / `./dot_scripts/uninstall_zsh.sh` — install or remove the Zsh package with Stow; history and plugin data are preserved on uninstall.
- `./dot_scripts/test_zsh.sh` — check syntax, XDG bootstrap, isolated startup without optional dependencies, and startup time.
- `./dot_scripts/configure_machine.sh` — Ubuntu/Debian/WSL-only bootstrap (apt packages, native Zsh config, ghostty, yazi, delta, GNOME tweaks on GUI). Exits on other distros.
...
- `./dot_scripts/install_zsh.sh` pins and installs `zsh-autosuggestions` and `zsh-syntax-highlighting` under `$XDG_DATA_HOME/zsh/plugins`; configuration must still start when they or `fzf` are absent.
